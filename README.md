...
**运行中的「更新物流信息到飞书表格-给达人发私信」可以取消（2026-09-18 起）：** 任务面板会显示「取消」。点它不会立刻杀进程：脚本把**手头这一行处理完**（该写的飞书行、该发的私信照常完成）才停，剩余行留到下次再跑；已写入飞书和已发出的私信都保留、不会回退，任务显示「已取消」并给出已处理行数。平台同意、详情页单条私信、自动审批写任务运行中仍不能取消——它们没有「整行」边界可停。

**生成待办报错 / 其它任务卡住：** 若达人资料连续出现 `code=100000` 或 `Please remove the plugin`，这是详情服务的系统级错误，不代表达人没有数据。补齐阶段连续 3 次命中后会停止本批类型 API 和详情页读取，仍可查询飞书语言；生成待办只用本轮已获得的资料，不会重新逐人补齐。类型缺失的待办保留人工确认，语言沿用飞书优先、简介其次、无值默认英语。先检查插件/浏览器环境或等待平台恢复，不要反复点击重跑整批。

若旧版本已经报 `database is locked`，等写任务结束后重新启动操作台以加载修复，再刷新网页；无需删除数据库或清空任务历史。新版本将飞书/详情读取和语言识别放在待办写事务之外，避免记录警告时等待自己持有的 SQLite 写锁。重启不会重新执行失败的批准或私信任务。
...
执行确认区分为「批准数量」和「飞书写入」两栏（窄屏上下排列）。网页新操作默认开启「批准后写入飞书」，需要只批准时可手动关闭；目标为「达人关系管理(新) / 达人管理总表」，仅平台批准成功后写入。恢复已有执行批次时显示该批次的实际设置，不套用新默认值。只读筛查不会写飞书，CLI/API 未显式启用时的默认行为不变，执行仍须输入 `y` 并最终确认。
...
- 指标阈值有前后端双重校验和上限；网页本次限量首次默认 20，修改后立即记忆有效正整数，同一浏览器、同一网址刷新或重新打开时恢复。没有有效记忆时回退 20；清空、0、负数或小数不会覆盖上次有效记忆。限量无业务上限，已选条数需要 ≤ 本次限量；不记忆名单和执行确认，CLI/API 默认限量仍为 1。
...
1. 页面打开（或点「重新扫描」）只读扫飞书，列出当前缺订单号的行；这一步不占用店铺页面、不写任何东西。
2. 输入 `y` 点 **补写订单号**：任务对每条候选到商家后台**按达人 ID 逐个搜索**（待发货 → 已发货 → 处理中 → 已完成 → 待审核），按「红人ID + 寄样产品」挑出对应申请的 `main_order_id` 后写飞书。只写「订单号」一列，不新建记录、不改合作状态或物流、不重新批准，已有不同订单号不覆盖。任务占用店铺页面（与其它页面任务互斥），运行中不可取消、不要重复点击。
3. 逐行结论写入「最近回读报告」（含订单号来源 tab、每个候选搜索失败的 tab）：`已写入并回读一致` / `飞书已有相同订单号` / `飞书已有不同订单号，转人工` / `写入结果未知，转人工` / `后台暂无订单号` / `后台存在多个订单号，转人工`。**任务完成不代表飞书已写入，以回读报告为准**；结果未知不自动重试。

只读整表扫描「待发货」会漏掉已经发货的记录，所以按达人逐个搜、并且查多个 tab：样品已寄出（在「已发货」）的行同样能补上订单号，不需要等第 8–9 步物流；仍然只写这一列，不发私信、不改合作状态。

默认不限量（按当前扫描结果，重复运行幂等）。后台还没把该达人转成待发货/已发货时会显示「后台暂无订单号」，等平台刷新后再点一次即可；记录留在 3 天窗口内会被自动重新扫到。飞书查询失败会直接报错，不会把「查不到」当成「缺失」。

下面是按历史批准批次的核对/补写（保留给脚本与维护用，页面不再提供）：

- `auto_approval_reconcile` 在 `write_feishu=false` 时只回读平台与飞书，无需 `y`；补写须 `y` + 15 分钟内成功核对的缺失证据 + 原批次限量，执行前重查、执行后回读。

查询失败不算缺失；重复记录、主键不一致、已有不同订单号、历史写入结果未知而本次未找到记录，均不能自动补写或覆盖。缺少执行备份、批准前就失败的批次会显示不可操作原因。核对任务失败保留原批准结果，不会把批准批次改成排队中；不要连续点击重试写入。

若旧批次显示 `queued`，关联的核对任务却已结束，这是历史状态异常，不是仍在运行。技术人员可用下面的维护入口先检查证据，再显式恢复本地状态：

```bash
python3 scripts/recover_auto_approval_state.py --execution-id <批次ID>
python3 scripts/recover_auto_approval_state.py --execution-id <批次ID> --apply
```

恢复只接受唯一成功的原批准任务、相符的执行信封及逐行身份/批准结论；有排队或运行任务时拒绝。应用前备份完整 SQLite 并保存恢复记录，只恢复批次状态、原批准任务关联和完成时间，不修改平台、飞书或逐行结果。恢复后先点「只读核对」，不能把旧核对成功当成新版回读证据，更不能重新批准。

当前平台确认沿用「待发货」核对口径。平台列表可能约 10 分钟才更新；已进入已发货、处理中或已完成阶段而无法在待发货确认的旧申请，保留待人工核对/后续物流处理，不能因待发货中未找到就重批或补建。
