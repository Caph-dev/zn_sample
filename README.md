...
操作台只做免费样品【处理中】与已发货物流的只读同步、指定 B005 的跟进待办和话术预览、CSV 导出。它不会批准或拒绝申请，不会发送私信，也不会写飞书合作状态、订单号或物流单号；只可按需只读飞书「使用语言」。这条只读物流同步不受北京时间 16:00 门限制；命令 3 的飞书回写和物流私信仍严格受 16:00 门及 execute 门闩约束。原 0/1/2/3 双击入口保持不变。

网页打开后，业务员优先点击 **开始今日更新**。它会按 SOP2 先同步物流和实际到货日，再生成 D0、D+3、D+7、D+10、D+15 本地跟进待办；页面会实时显示当前步骤、进度、警告，结束后显示读取数、更新数和下一步入口。页面中的「只同步物流」「只生成跟进待办」是排查时使用的单独操作。网页不会批准申请、写飞书或发送任何私信。
