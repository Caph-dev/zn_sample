...
启动时会自动停止仍在运行的旧实例再启动，保证加载最新代码；运行中的网页写任务（打开店铺 / 1、2、3 入口）会阻止自动重启，避免中断平台写操作。旧实例存在时只想打开浏览器，用 `--no-restart`。

操作台包含两组能力：

1. **只读日常更新**：免费样品【处理中】与已发货物流同步、全部主推款的跟进待办和话术预览、CSV 导出。这组操作不会批准申请、发送私信或写飞书，且只读物流同步不受北京时间 16:00 门限制。
2. **筛查与执行**：网页先提供独立的“打开店铺”准备操作，再列出 1、2、3 三个脚本入口。1 只出名单；2、3 会在明确确认后执行批准、飞书写入和私信等对应写操作。2 仍最多 50 条并等待平台 10 分钟，3 仍受北京时间 16:00 门限制（提前运行必须再次输入 `FORCE`）。
...
首页把这些入口统一放在 **筛查、物流与跟进** 区：“打开店铺”独立放在“运行前准备”，其后是 1–3 脚本，最后是第二份 SOP 对应的“到货与跟进（只读）”。2、3 会打开确认对话框，必须输入 `y`；3 在北京时间 16:00 前还会要求再次输入大写 `FORCE`。运行 1、2、3 时只保留一家店，并且不要手动点击店铺页面。首页的完整数据目录、数据库与运行店铺信息默认收在 **运行环境** 折叠项中。

“运行前准备”下方会显示当前打开的店铺和调试口状态。调试口以 `execute_script` 实际探活为准：就绪后“打开店铺”按钮变为浅灰色，表示可以进入下一步，但按钮仍可点击以重新打开；未就绪时按钮保持高亮。状态检查只读，不会自动开店或重开店铺。
