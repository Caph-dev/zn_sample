...
自动批准页始终显示「核对与补写」，与当前筛查名单分开。可分页选择历史批准批次，刷新页面或重新筛查不会丢失旧批次入口。

1. 先选批次，核实店铺及原批次限量，再点 **只读核对**。不需要输入 `y`，不批准、不写飞书；会占用店铺页面，不能和其他页面任务并发。
2. 查看逐行结果：平台确认、飞书达人关系主键（红人ID + 寄样产品）、订单号。**任务完成不代表飞书已写入**；只有实际回读一致才显示已核对，接口返回成功仅是历史执行结果。
3. 有可安全补写的缺失项时，输入 `y` 执行 **补写缺失项**。即使原批准批次关闭了写飞书，也可在这里单独明确授权；沿用原批次限量，不重新批准、不发私信、不修改合作状态或物流字段。
4. 补写前再次核对，补写后再次回读。核对报告超过 15 分钟须重新核对；历史批准批次的核对不受原筛查 24 小时有效期限制，但不能凭过期筛查发起新批准。
