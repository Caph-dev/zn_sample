...
- 语言：先读飞书「达人关系管理(新)」**使用语言**（英语 / 西班牙语）。无值再走达人详情简介 `detect_creator_lang`：有简介时用 LLM JSON 的 `lang`（不看 `confidence`）；空简介、识别失败或非法 lang 默认英语。
- 话术区分英语/西班牙语、视频达人/直播达人。视频+直播同时标记 → 跟进私信统一按视频达人话术，不拆两条发送。
