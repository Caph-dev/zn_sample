...
飞书 wiki 正文里仍可能出现旧日历（到货第五天、每隔两天）和 B006-A。**跟进日历、配图商品、未履约**以本节和业务员说明为准，忽略 B006-A。
...
- **【已发货】** = 在途，不是到货。
- 当前 **批准和跟进都只过** 商品 ID `1732414717062320994`（货号 B005 且主推=是的那一款）。其它货号不进跟进池。恢复全部主推：筛查加 `--all-hero-products`。非该 B005 的到货话术原文仍留在下方，供以后打开跟进时用，当前不发送。
...
## 如果不是该 B005（其它货号；当前不进跟进池，原文留给以后）：
